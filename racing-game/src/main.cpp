...
extern "C" void game_update(float dt)
{
    ...
    float steer = 0.0f;
    if (g->api->is_key_down(KEY_D)) steer -= 1.0f;
    if (g->api->is_key_down(KEY_A)) steer += 1.0f;
    ...
    // ---------------------------------------------------------------------------
    glm::vec3 forward{
        std::sin(car->rotation[1]),
        0.0f,
        std::cos(car->rotation[1])
    };
    ...
    glm::vec3 look = glm::normalize(car_pos - glm::vec3(g->camera.position[0], g->camera.position[1], g->camera.position[2]));
...
{
    if (id == g->local_client_id) return;
