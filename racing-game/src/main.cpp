...
extern "C" game_info_t* game_get_info() {
  static game_info_t info{
    .abi_version = OASIS_GAME_ABI_VERSION,
    .game_id     = "racing-game",
    .name        = "Racing Game",
    .version     = "1.0.0",
...
static void set_sky_clear_color() {
  if (!g_api || !g_api->clear_color) return;
...
extern "C" void game_update(float dt) {
  ...
  float steer = 0.0f;
  if (g_api->is_key_down(KEY_D)) steer -= 1.0f;
  if (g_api->is_key_down(KEY_A)) steer += 1.0f;
  ...
  // ---------------------------------------------------------------------------
  glm::vec3 forward{
    std::sin(car.rotation),
    0.0f,
    std::cos(car.rotation)
  };
  ...
  glm::vec3 desired =
    car_pos - forward * FOLLOW_DIST + glm::vec3(0, FOLLOW_H, 0);

  for (int i = 0; i < 3; ++i)
    g_camera.position[i] +=
      (desired[i] - g_camera.position[i]) * std::min(1.0f, CAM_DAMP * dt);

  glm::vec3 look = glm::normalize(car_pos -
    glm::vec3(g_camera.position[0], g_camera.position[1], g_camera.position[2]));
...
extern "C" void game_on_client_join(uint32_t client_id) {
    ...
    // Ignore local client
    if (client_id == g_local_client_id) return;

    if (std::any_of(g_entities.begin(), g_entities.end(),
...
extern "C" void game_on_local_client_ready(uint32_t client_id) {
  if (g_local_client_id != UINT32_MAX) return;
